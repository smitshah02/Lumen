...
(src/mcp_server/planes.py) and LUMEN_HNSW_EF_SEARCH are all read at *import*
time, so a load_dotenv() inside main() would arrive too late for all of them.

Before this existed, nothing outside src/reranker/audit_training_quality.py
called load_dotenv(), so .env was inert: tracing was silently off, the MIMIC
data dirs were unset, and DATABASE_URL worked only because the hardcoded
fallback in src/storage/__init__.py happened to match the file.

override=False on purpose — a variable already exported in the shell beats
.env, so `LUMEN_HNSW_EF_SEARCH=200 python -m src.evals.eval_retrieval` still
